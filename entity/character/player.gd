...
func get_collision_interact_root(ptag):
	while ptag and ptag is Node3D:
		if ptag is Entity_Build:
			return ptag
		ptag = ptag.get_parent()
	return null
...
func set_interact_target(tag,b):
	if b:
		if interact_target==tag:
			return
		tag=get_collision_interact_root(tag)
		if not tag:
			return
		interact_target=tag
	else:
...
func ref_available_actions():
	...
	if phold:
		available_tag_actions=phold.get_canbe_actions()
	if interact_target:
		available_tag_actions.append_array(interact_target.get_canbe_actions())
...
func flip():
	if not interact_target:
		return
	interact_target.on_flip()

func discard():
	if not interact_target:
		return
	interact_target.on_discard()
